...
increment = 1.5 # Pas de changement de l'angle
increment_claw=2.5
...
def check_finger_positions(hand_landmarks):
    ...
    #Calculate x angle
    angle = int(((WRIST.x - INDEX_FINGER_MCP.x) /palm_size) * 180 / 3.1415926)
    angle = clamp(angle, palm_angle_min, palm_angle_mid)
    servo_angle0= map_range(angle, palm_angle_min, palm_angle_mid, 0, 180)



    distances = {
        "index": calculate_distance(WRIST, INDEX_FINGER_TIP) > calculate_distance(WRIST, INDEX_FINGER_DIP),
        "middle": calculate_distance(WRIST, MIDDLE_FINGER_TIP) > calculate_distance(WRIST, MIDDLE_FINGER_DIP),
        "pinky": calculate_distance(WRIST, PINKY_TIP) > calculate_distance(WRIST, PINKY_DIP),
        "thumb": calculate_distance(THUMB_TIP, INDEX_BASE) > calculate_distance(THUMB_BASE, INDEX_BASE),
        "ring": calculate_distance(WRIST, RING_TIP) > calculate_distance(WRIST, RING_DIP),
        "servo0":90>=servo_angle0>=0,
        "autorisation": palm_size>=0.1568279321240721
    }
    return distances
...
with mp_hands.Hands(model_complexity=0, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
 ...
 # Initialisation de la fenêtre
    cv2.namedWindow("Test Servo", cv2.WINDOW_NORMAL)
    cv2.setWindowProperty("Test Servo", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks and results.multi_handedness:
            ...
            if right_hand_landmarks and left_hand_landmarks:
                ...
                if distances_right.get("autorisation"):
                    if distances_right.get("servo0")==0:
                       new_angle=servo_angle[0]+increment 
                    else:
                        new_angle=servo_angle[0]-increment
                    ...
                    if new_angle != servo_angle[0]:
                        servo_x_pin.write(new_angle)
                        servo_angle[0] = new_angle
                ...
                if distances_left.get("thumb"):  # Exécution autorisée uniquement si le pouce gauche est levé
                    ...
                    if right_thumb_up:
                        # Si le pouce droit est levé, incrémenter l'angle
                        new_angle = servo_angle[5] + increment_claw
                    else:
